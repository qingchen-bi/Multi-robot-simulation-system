...
// Developed by bqc NKU on 2023/11/15
//
...
SlamOutput::SlamOutput(const ros::NodeHandle &nh, const ros::NodeHandle &nh_private) : nh_(nh), nh_private_(nh_private),
    ...
    // 订阅局部点云和里程计消息
    local_cloud_sub_.reset(new message_filters::Subscriber<sensor_msgs::PointCloud2>(nh_, "velodyne_points", 1));
    local_odom_sub_.reset(new message_filters::Subscriber<nav_msgs::Odometry>(nh_, "base_pose_ground_truth", 100));
    ...
    execution_timer_ = nh_.createTimer(ros::Duration(0.5), &SlamOutput::execute, this); // 核心函数
...
void SlamOutput::pointCloudOdomCallback(const sensor_msgs::PointCloud2ConstPtr &scanIn,
    ...
    broadcaster.sendTransform(ST_B_Bi); // 发布到TF map-sensor 的坐标变换 100Hz
    ...
    odom_pub.publish(odom_msg); // laser_odom_init 激光里程计
...
{   
    if(is_get_first)
    {
        ...
        // scan_dwz_msg.header= scanIn_->header;
        // dwz_cloud_pub.publish(scan_dwz_msg);// publish downsample point cloud
        
        tf::StampedTransform T_b_bi = ST_B_Bi_;
        ...
        pose << T_b_bi.getBasis()[0][0], T_b_bi.getBasis()[0][1], T_b_bi.getBasis()[0][2], T_b_bi.getOrigin()[0],
                ...
                0, 0, 0, 1;
        // Slam-sim-out 拼接 局部坐标系转化到了全局坐标系下
        ...
        *exploredAreaCloud += *registered_scan;
        ...
        scan_data_msg.header.seq = scanIn_->header.seq;
        reg_pub.publish(scan_data_msg); // 发布配准之后的点云，配准是在降采样之后作的 拼接
    }
